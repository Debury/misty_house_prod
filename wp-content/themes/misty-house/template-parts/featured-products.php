...
if ( $products->have_posts() ) {
    ...
    // No real products: generate placeholders
    for ( $i = 0; $i < 10; $i++ ) {
        ob_start();
        ?>
        <div class="tshirt-item placeholder">
          <div class="tshirt-image-wrapper">
            <img src="<?php echo esc_url( $placeholder ); ?>" alt="Placeholder T-shirt">
          </div>
        </div>
        <?php
        $items_html[] = ob_get_clean();
...
<section class="tshirts-section">
  <div class="tshirts-carousel-wrapper">
    ...
    <div class="tshirts-container" id="tshirts-container">
      <?php
      // First pass: output every item
      foreach ( $items_html as $html ) {
          echo $html;
      }

      // Second pass: clone only if there's more than one item
